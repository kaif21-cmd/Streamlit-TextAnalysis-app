...
class SpellingCorrection:
    ...
    def get_explanation(self)->tuple[dict,list]:
        ...
        mistake_mask:list[bool] = [True if a!=b else False for a,b in zip(input_text, correct_text)]
        response:dict = {}
        for i, mistake in enumerate(mistake_mask):
            if mistake:
                ...
                response[word_mistake] = {'correction':m.spellcheck(), 'definition': w.definitions} 
...
class PartsOfSpeechTagging:
    ...
    def part_of_speech_help(self)->dict[str:str]:
        pos_txt:list = pathlib.Path('./parts-of-speech-tags.txt').read_text().splitlines()
        pos_tag_map:dict[str:str] = dict([ tuple(txt.split(sep='\t')) for txt in  pos_txt[2:] ])
        return pos_tag_map
...
class TextSummarize:
    ...
    def unique_token_frequency_scaled_map(self,)->dict[str:float]:
        ...
        token_frequency_scaled:dict[str:float] = {k:v/W for k,v in token_frequency.items()}
    ...
    def get_token_frequency_df(self,)->pd.DataFrame:
        weights:dict[str:float] = self.unique_token_frequency_scaled_map()
        df = pd.DataFrame(data={'Tokens':weights.keys(),'Weights':weights.values()})
        return df
